 ...
 * https://r.revoplus.ru/ - адрес для `production` работы сервиса.
 * https://demo.revoup.ru/ - адрес сервиса для тестирования и отладки.
...
1. На стороне Рево формируются уникальный идентификатор сайта магазина и секретный ключ, которые далее передаются партнеру
 * `store_id`(уникальный идентификатор сайта магазина) - уникальный идентификатор сайта. Для одного партнера может быть сформировано несколько уникальных идентификаторов.
 * `secret_key`(секретный ключ) - ключ используется при формировании электронно-цифровой подписи (ЭЦП) для аутентификации (проверка подлинности) параметров запроса с целью защиты формы от запуска сторонними лицами. Длина ключа от 8 байт. Алгоритм шифрования SHA1.
2. Для авторизации в сервисах Рево Партнёром используется <a href="#1c37860b3b">цифровая подпись</a> и уникальный идентификатор магазина. Партнер, отправляет `POST` запрос используя уникальный идентификатор магазина `store_id` и сформированую <a href="#1c37860b3b">цифровую подпись</a>. 
...
Метод возвращает адрес iFrame с процедурой аутентификации или регистрации клиента для оформления заказа.

* Если номер телефона клиента не найден в базе Рево (новый клиент), то в `callback_url` будет передана ссылка на полную форму, которая начинается с регистрации.
* Если номер телефона клиента найден в базе Рево (повторный клиент), то по нему будет принято решение о выдаче лимита. ?? и что в ответе при этом ??
