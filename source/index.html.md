...
<aside class="notice">
Необходимо обратить внимание, что при формировании signature всегда будет 40 символов по SHA1.
</aside>
...
Метод предназначен для оформления заказа как новых, так и повторных клиентов. Метод возвращает адрес iFrame с формой для оформления заказа - `iframe_url`:

* Если номер телефона клиента не найден в базе Рево (новый клиент), то будет передана ссылка на форму для регистрации и дальнейшего оформления заказа.
* Если номер телефона клиента найден в базе Рево (повторный клиент), то будет передана ссылка на форму для аутентификации и дальнейшего оформления заказа. ??Как будет принято решение о выдаче лимита. ?? и что в ответе при этом ??
